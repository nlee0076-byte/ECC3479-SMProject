...
### The main result is robust across most specifications. In the baseline model, the coefficient on social media usage is 1.7653 (SE = 0.2344), and the coefficient remains positive and statistically significant in nearly all alternative models.The no-controls model produces a larger coefficient of 2.4466, suggesting that some controls absorb part of the relationship, but the effect remains highly significant after controls are included. In other words, even after removing extra variables like age or sleep, the main relationship between social media usage and academic performance still stays strong. The alternative controls specification gives a very similar estimate of 1.8825, showing that the result is not highly sensitive to the exact control set used. Removing outliers has almost no impact on the result, with the coefficient remaining at 1.7651 after excluding 3 observations. This suggests the finding is not driven by extreme values. In the undergraduate-only subsample, the coefficient falls to 0.9414 but remains statistically significant, indicating the relationship still holds within a narrower sample. The linear probability model also produces a positive and significant estimate of 0.1632, while the log specification remains strongly positive at 7.5449. These results suggest the finding is not dependent on the estimation method or scaling of the explanatory variable.The quadratic specification is the only model where the coefficient becomes statistically insignificant (-0.7883), suggesting that the relationship may become nonlinear at higher levels of social media usage. Overall, the consistency in sign and significance across most specifications strengthens the credibility of the main finding. The finding survives multiple alternative specifications and does not appear to be driven by one particular modelling choice.
